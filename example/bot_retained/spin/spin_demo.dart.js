...
$$.CanvasRenderingContext2D = {"": "CanvasRenderingContext;fillStyle},globalAlpha%,strokeStyle}",
...
$.main = function() {
  var demo = $.SpinDemo_SpinDemo(document.querySelector("#content"));
...
$.SpinDemo_SpinDemo = function(canvas) {
  var pCanvas, blue, green, red, t1, t2, tx, t3, rootPanel;
  ...
  t2 = $.Coordinate$(15, 15);
  $.get$PanelThing__containerTransformProperty().get$1(green).setToTranslation$2(t2.x, t2.y);
  if (green._disposed)
    $.throwExpression($.DisposedError_SMD);
  ...
  t1 = pCanvas._disposed;
  if (t1)
    $.throwExpression($.DisposedError_SMD);
  tx = $.AffineTransform$(1, 0, 0, 1, 0, 0);
  t2 = pCanvas._transforms;
  t2.push(tx);
  t3 = $.getInterceptor$x(canvas);
  tx.translate$2(tx, $.$div$n($.$sub$n(t3.get$width(canvas), pCanvas.get$width(pCanvas)), 2), $.$div$n($.$sub$n(t3.get$height(canvas), pCanvas.get$height(pCanvas)), 2));
  if (t1)
    $.throwExpression($.DisposedError_SMD);
  tx = $.AffineTransform$(1, 0, 0, 1, 0, 0);
  t2.push(tx);
...
$.RetainedDebug__borderThingCore = function(ctx, thing) {
  ...
  for (t2 = t2.get$iterator(corners); t2.moveNext$0() === true;) {
    ...
    t1.lineTo$2(ctx, t3.get$x(p), t3.get$y(p));
  }
  t1.stroke$0(ctx);
};
